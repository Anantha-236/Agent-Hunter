...
_BUSINESS_IMPACT: Dict[str, Dict[str, Any]] = {
    ...
    "cors_scanner": {
        "assets": ["authenticated API responses", "session-bound user data", "cross-origin trust boundary"],
        "blast_radius": "all browser users with active sessions",
        "exploitation_base": 0.72,
        "chain_enables": ["cross_origin_data_theft", "csrf_chaining", "account_data_exposure"],
        "owasp_risk": "A05:2021 – Security Misconfiguration",
        "cvss_base": 7.1,
        "urgency": "HIGH — Credentialed CORS errors can expose user data at scale",
    },
    "header_security": {
        "assets": ["browser trust boundaries", "session integrity", "client-side security posture"],
        "blast_radius": "all users interacting with affected pages",
        "exploitation_base": 0.48,
        "chain_enables": ["clickjacking", "mime_sniffing_abuse", "xss_impact_amplification"],
        "owasp_risk": "A05:2021 – Security Misconfiguration",
        "cvss_base": 5.0,
        "urgency": "MEDIUM — Fast remediation and high defensive value",
    },
    "sensitive_data_exposure": {
        "assets": ["credentials", "private keys", "deployment secrets", "internal configs"],
        "blast_radius": "service-wide and potentially multi-environment",
        "exploitation_base": 0.84,
        "chain_enables": ["account_takeover", "privilege_escalation", "supply_chain_abuse"],
        "owasp_risk": "A02:2021 – Cryptographic Failures",
        "cvss_base": 8.8,
        "urgency": "CRITICAL — Secret leakage can lead to immediate compromise",
    },
    "jwt_scanner": {
        "assets": ["authentication tokens", "session authorization boundaries", "user identity assertions"],
        "blast_radius": "all users sharing vulnerable token validation logic",
        "exploitation_base": 0.82,
        "chain_enables": ["auth_bypass", "account_takeover", "privilege_escalation"],
        "owasp_risk": "A07:2021 – Identification and Authentication Failures",
        "cvss_base": 8.6,
        "urgency": "HIGH — JWT validation flaws can bypass auth entirely",
    },
    "rate_limit_scanner": {
        "assets": ["login endpoints", "otp flows", "account recovery surfaces", "API quotas"],
        "blast_radius": "all exposed auth and verification endpoints",
        "exploitation_base": 0.64,
        "chain_enables": ["credential_stuffing", "otp_bruteforce", "user_enumeration"],
        "owasp_risk": "A04:2021 – Insecure Design",
        "cvss_base": 6.8,
        "urgency": "HIGH — Missing throttling increases account takeover risk",
    },
    "broken_access_control": {
        "assets": ["tenant data", "admin functions", "user-owned resources"],
        "blast_radius": "cross-user and potentially cross-tenant",
        "exploitation_base": 0.78,
        "chain_enables": ["horizontal_escalation", "vertical_escalation", "data_tampering"],
        "owasp_risk": "A01:2021 – Broken Access Control",
        "cvss_base": 8.2,
        "urgency": "HIGH — Access checks failing beyond IDOR indicates systemic risk",
    },
    "lfi_rfi_scanner": {
        "assets": ["application source code", "system files", "runtime secrets"],
        "blast_radius": "single host to full environment depending on include behavior",
        "exploitation_base": 0.83,
        "chain_enables": ["secret_extraction", "rce", "lateral_movement"],
        "owasp_risk": "A05:2021 – Security Misconfiguration",
        "cvss_base": 8.6,
        "urgency": "CRITICAL — Include flaws often bridge to code execution",
    },
    "ssl_tls_scanner": {
        "assets": ["transport confidentiality", "session integrity", "credential exchange"],
        "blast_radius": "all users on affected TLS endpoints",
        "exploitation_base": 0.58,
        "chain_enables": ["mitm", "downgrade_attack", "credential_interception"],
        "owasp_risk": "A02:2021 – Cryptographic Failures",
        "cvss_base": 6.5,
        "urgency": "MEDIUM — Transport hardening needed to prevent interception",
    },
    "open_redirect": {
